...
int *SKloginsystem_main(int *Tsab)
{
  ...
  printf("\n\t\t\t-----------Welcome To Arduino Education Login Page-----------\n");
  while (SKa == 0)
  {
    printf(
      ...
      while (fread(&Tuserf, sizeof(struct Tuser), 1, Tfile2))
      {
        ...
        if (!strcmp(Tuserf.user_name, Tsinguser))
        {
          if (!strcmp(Tuserf.password, Tsingpass))
          {
            ...
            printf("\n\t\t\t---------Welcome To Arduino Education %s---------", Tuserf.name);
    ...
    case 2:
      ...
      if (fwrite != 0)
      {
        ...
        cursor_coordinator(0, 0);
        printf("\n\t\t\t---------Welcome To Arduino Education %s---------", Tuserinf.name);
...
void SKarduinoConnection()
{
  ...
  {
    printf("\n\nYour arduino has connected succesfully. Welcome to ..."); //Three dots will be filled with the project name
...
{   
  sleep(1);
    ...
    if (SKchoice == '1')
    {
        ...
        sleep(3);
        system("Arduino_Traffic_Light.png");
        SKexpChange+=10;
        while (SKi == 1)
        {
            system("cls");
